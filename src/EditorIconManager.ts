...
export default class EditorIconManager extends IconManager {
	constructor(plugin: IconicPlugin) {
		...
		this.setMutationObserver(
			...
			(mutations) => {
				if (
					!activeDocument.activeElement?.hasClass(
				...
				for (const mutation of mutations) {
					for (const addedNode of mutation.addedNodes) {
						if (
							...
							addedNode.hasClass("suggestion-container")
						) {
	...
	private observeProperties(propsEl: HTMLElement) {
		...
		this.setEventListener(
			...
			"click",
			(event) => {
				const pointEls = activeDocument.elementsFromPoint(
					event.x,
					event.y
				);
				...
				if (iconEl && propEl instanceof HTMLElement) {
					const prop = propEl.dataset.propertyKey
						? this.plugin.getPropertyItem(
								propEl.dataset.propertyKey
						  )
						: null;
					if (!prop) return;
	...
	private refreshViewIcons(props: PropertyItem[], view: MarkdownView) {
		...
		for (const propEl of propEls) {
			const prop = props.find(
				(prop) => prop.id === propEl.dataset.propertyKey
			);
			if (!prop) return;
	...
	private onSuggestionMenu(suggestMenuEl: HTMLElement): void {
		...
		const propEls = suggestMenuEl.findAll(
			":scope > .suggestion > .suggestion-item"
		);
		for (const propEl of propEls) {
			const propId = propEl
				.find(":scope > .suggestion-content > .suggestion-title")
				?.getText();
			if (propId) {
				const prop = this.plugin.getPropertyItem(propId);
				const iconEl = propEl.find(
					":scope > .suggestion-icon > .suggestion-flair"
				);
				if (iconEl) this.refreshIcon(prop, iconEl);
			}
		}
